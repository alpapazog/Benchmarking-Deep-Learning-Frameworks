...
# ✅ Load and pre-tokenize IMDB test dataset (CPU side preprocessing)
print('Loading and pre-tokenizing IMDB test dataset (CPU)...')
(_, test_data), _ = tfds.load(
    'imdb_reviews',
    split=['train', 'test'],
    as_supervised=True,
    with_info=True
...
# ✅ Load the model (custom model automatically found due to decorator)
print('Loading model...')
with tf.device(device):
    model = tf.keras.models.load_model('sentiment_lstm_tf_saved.keras')
...
# ✅ Get peak GPU memory usage (if applicable)
